"""Generate mesh.npz input files for all examples.

Run once to produce the mesh files consumed by the generated Taichi solvers:

    uv run python examples/gen_meshes.py
...
def merge_bcs(
    *parts: tuple[np.ndarray, np.ndarray],
    ...
    for dofs, vals in parts:
        for d, v in zip(dofs.tolist(), vals.tolist(), strict=True):
            if d not in seen:
                seen[d] = v
    sorted_items = sorted(seen.items())
    if not sorted_items:
        return np.array([], dtype=np.int64), np.array([], dtype=np.float64)
    d_arr, v_arr = zip(*sorted_items, strict=True)
