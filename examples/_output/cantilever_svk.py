...
# ======================================================================
# Constitutive model: svk
# ======================================================================

@ti.func
def constitutive_update(F: ti.types.matrix(3, 3, ti.f64),
                        lam: ti.f64, mu: ti.f64
                        ) -> ti.types.matrix(3, 3, ti.f64):
    """SVK constitutive model: S = lam*tr(E)*I + 2*mu*E."""
    # Right Cauchy-Green tensor C = F^T @ F
    C = F.transpose() @ F
    # Green-Lagrange strain E = 0.5*(C - I)
    I3 = ti.Matrix.identity(ti.f64, 3)
    E = 0.5 * (C - I3)
    # Trace of E (physics index sum -- ti.static range)
    tr_E = ti.f64(0.0)
    for i in ti.static(range(3)):
        tr_E += E[i, i]
    # 2nd Piola-Kirchhoff stress: S = lam*tr(E)*I + 2*mu*E
    S = lam * tr_E * I3 + 2.0 * mu * E
    return S
    ...
    for e in range(n_elem):
        ...
        for q in ti.static(range(N_QP)):
            ...
            if detJ0 > 1e-15:
                ...
                # Constitutive update: S = constitutive_update(F, lam, mu)
                S = constitutive_update(F, lam, mu)
...
def tangent_matvec(v_flat: np.ndarray, lam: float, mu: float) -> np.ndarray:
    ...
    for e in range(n_elem):
        ...
        for q in range(N_QP):
            ...
            # SVK PK2 stress: S = lambda tr(E) I + 2 mu E.
            tr_E = float(np.trace(E))
            S = lam * tr_E * I3 + 2.0 * mu * E

            # SVK material tangent C is constant; the contraction C : dE
            # collapses to the same closed form as the stress update.
            tr_dE = float(np.trace(dE))
            dS = lam * tr_dE * I3 + 2.0 * mu * dE
...
def validate_mesh() -> None:
    ...
    for e in range(n_elem_val):
        X_e = coords_np[conn_np[e]]
        for q in range(N_QP):
            dN = np.array(GRAD_AT_QUAD[q])
            J0 = X_e.T @ dN
