...
_TOKEN_RE = re.compile(
    '|'.join(f'(?P<T{i}>{pat})' for i, (pat, _) in enumerate(TOKEN_PATTERNS))
)
...
def tokenize(latex: str) -> list[Token]:
    ...
    for m in _TOKEN_RE.finditer(latex):
        for i, (_, name) in enumerate(TOKEN_PATTERNS):
            g = m.group(f'T{i}')
            if g is not None:
                if name == 'WS' or name == 'THINSPACE':
                    break   # skip whitespace
                val = g
                # Extract inner text for styled commands
                if name == 'STYLED' or name == 'OPNAME':
                    inner = re.match(TOKEN_PATTERNS[0 if name == 'STYLED' else 1][0], g)
                    if inner:
                        val = inner.group(1)
                if name == 'GREEK':
                    inner = re.match(TOKEN_PATTERNS[list(n for _, n in TOKEN_PATTERNS).index('GREEK')][0], g)
                    if inner:
                        val = inner.group(1)
...
class ExprParser:
    ...
    def expect(self, kind: str) -> Token:
        ...
        if tok is None or tok.kind != kind:
            ...
            raise SyntaxError(
                f"Expected {kind} but got {got} at position {self.pos}"
            )
        return self.advance()
    ...
    def _can_start_factor(self) -> bool:
        ...
        if tok is None:
            return False
        return tok.kind in (
            'LETTER', 'GREEK', 'STYLED', 'NUMBER', 'LPAREN',
            'FRAC', 'SQRT', 'LNORM', 'OPNAME'
        )
    ...
    def parse_superscript(self, base: Expr) -> Expr:
        ...
        if self.at('LBRACE'):
            ...
            # Check for ^{T}
            if self.at('LETTER') and self.peek().value == 'T':
                saved_pos = self.pos
                self.advance()
                if self.at('RBRACE'):
                    self.advance()
            ...
            if self.at('MINUS'):
                saved_pos = self.pos
                self.advance()
                if self.at('NUMBER') and self.peek().value == '1':
                    self.advance()
                    self.expect('RBRACE')
                    # Mark as inverse — used for M^{-1}(r)
    ...
    def parse_base(self) -> Expr:
        ...
        # Norm: \| expr \| or \lVert expr \rVert
        if self.at('NORMPIPE', 'LNORM'):
            return self.parse_norm()

        # Atom (may be followed by parens → func call)
        atom = self.parse_atom()

        # Check for function call: atom(...) or inverse(...)
        if self.at('LPAREN'):
            self.advance()
            args = []
            if not self.at('RPAREN'):
        ...
        if self.at('LBRACE'):
            ...
            while depth > 0:
                ...
                if tok.kind == 'LBRACE':
                    ...
                    if depth == 0:
                        break
                # Extract text from styled commands
                if tok.kind == 'STYLED':
                    parts.append(tok.value)
                elif tok.kind in ('LETTER', 'GREEK', 'NUMBER'):
                    parts.append(tok.value)
                elif tok.kind == 'PLUS':
                    parts.append('+')
                elif tok.kind == 'MINUS':
...
def parse_assignment(latex: str) -> tuple[Var, Expr] | None:
    ...
    """
    # Split on first '=' that isn't inside braces
    depth = 0
    eq_pos = -1
...
def parse_condition(latex: str) -> Expr:
    ...
    # No comparison found — treat as a boolean expression
