...
class TypeInferrer:
    ...
    def _infer_expr(self, expr: Expr) -> VarType:
        ...
        if isinstance(expr, FuncCall):
            ...
            # Function call on a callable → infer from args
            # M^{-1}(r) where r is vector → result is vector
            if expr.args:
    ...
    def _resolve_binop(self, expr: BinOp, lt: VarType, rt: VarType) -> VarType:
        ...
        if op in ('+', '-'):
            # Addition/subtraction: result type = dominant operand type
        ...
        if op == '/':
            # Division: typically scalar / scalar
            return VarType.SCALAR
    ...
    def _resolve_multiply(self, expr: BinOp, lt: VarType, rt: VarType) -> VarType:
        ...
        """
        # Check for dot product: v^T * w
        left_is_transposed = (
            isinstance(expr.left, UnaryOp) and expr.left.op == 'transpose'
        )
        ...
        if left_is_transposed and rt == VarType.MATRIX:
            # v^T A  — row vector times matrix, still need to see what follows
            expr.op = 'dot'
            return VarType.VECTOR
