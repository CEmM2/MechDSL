...
# Absolute floor is the P8-3.json criterion (kept for traceability); the
# relative term is what actually carries the test when |u_ref| is small.
    ...
    * ``vertices`` — node count + spatial dim + one ``x y z`` row per node.
    ...
    def _face_attribute(face_nodes: tuple[int, int, int, int]) -> int | None:
        ...
        if np.allclose(xs, 0.0):
            return 1  # root (Dirichlet)
        if np.allclose(xs, _CANTILEVER_LX):
            return 2  # tip (Neumann)
...
# output so we do not have to edit ``mfem_printer.py`` (Gate-B approved).
#
# Phase 8 Gate-B attempt 3 update: instead of compiling this as a separate
# translation unit (which would require a CMake edit the printer does not
# emit), we splice this free function definition into the printer's
# ``main.cpp`` *immediately before* ``int main(...)``.  That keeps it in a
# single TU and avoids touching the Gate-B-approved CMakeLists.txt.  The
# injected ``main`` body then calls it via the ``MECHDSL_DISP_OUT`` env
# var so we also do not need to teach the printer's OptionsParser a new
# ``--out`` flag.
_MFEM_DISP_DUMP_CPP = """\
