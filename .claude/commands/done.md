...
Write a condensed handoff file to `dev/chats/<project-name>_<YYYY-MM-DD>_<HH-MM>_handoff.md` for next-session resumption. This file is read by a SessionStart hook so the next session has immediate context.
...
Overwrite any existing `<project-name>_<YYYY-MM-DD>_<HH-MM>_handoff.md` — only the most recent session's handoff matters.
...
Logged to: Laptop-session-log.md
...
1. **Prune old entries:** Scan `dev/session-log.md` for entries older than 60 days. Move them  to `dev/session-log-archive.md`.

2. **Log performance:**
   ```bash
   echo "$(date +%Y-%m-%d),done,TOOL_CALLS,NOTES" >> dev/skill-performance.csv
   ```
