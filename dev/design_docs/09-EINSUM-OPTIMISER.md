...
The current tier system (Tier 1/2/3) is sufficient for the MVP's Hex8 element and power-law elasto-plasticity. For Plan B, when the project supports multiple element types (Tet10, Hex20, reduced-integration Hex8) and more complex constitutive models (viscoplasticity with state-update contractions, convected-coordinate transforms), the tier system will evolve into **named contraction-family templates**.

### 9.1  Planned template families

| Family | Description | Current tier equivalent |
|--------|-------------|----------------------|
| `dot_reduction` | Inner product / trace-like reduction | Tier 1 |
| `matvec_small_static` | Matrix-vector with static dims ≤ 6 | Tier 1 |
| `gemm_small_static` | Matrix-matrix with static dims ≤ 6 | Tier 1 |
| `batched_gemm_like` | Push-forward / pull-back style contractions | Tier 2 |
| `scale_accumulate` | Weighted scatter / accumulation | Tier 2 |
| `load_projection` | Basis function projection | Tier 2 |
| `fallback` | Anything exceeding budget | Tier 3 |

### 9.2  Transition plan

The tier system will remain the **scheduling decision** (how much to unroll). Template families will become the **realisation decision** (what shape the emitted code takes). The two are complementary:

1. Einsum optimiser produces a `ContractionPlan` with tier assignments (unchanged).
2. Backend scheduler maps each tier-2 step to a named template family based on the contraction's shape.
3. Template emitters generate the specific Taichi loop nests for each family.

This separation keeps the optimiser backend-neutral while allowing the Taichi emitter to specialise its output per contraction shape. The transition happens during Plan B alongside the introduction of additional element types (B5) and backend printers (B8), and may warrant a dedicated Plan B phase for contraction template tuning.
