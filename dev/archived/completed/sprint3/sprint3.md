...
> ⚠️ **Superseded** by [`recovery_plan_latex_contract.md`](recovery_plan_latex_contract.md) (Phase 7 / R6 archival, P7-5). The benchmark / acceptance work here is preserved in tree; the implicit LaTeX-input acceptance test has moved to recovery-plan Phase 7 (R6). See [`STATUS_LEGEND.md`](../tracking/STATUS_LEGEND.md) and [`frontend_drift_history.md`](../reviews/frontend_drift_history.md).
...
- Rigid body 30-degree rotation (`test_patch_test.py:374` -- irregular mesh, force_norm < 1e-12)
...
| P1-T4 | Add 30-degree finite rotation test to `TestRigidBodyMotion` in test_benchmarks.py (using reference solver directly) | `tests/test_benchmarks.py` | S |
...
**P3-T3 details -- reference regression snapshot:**
- Run `ref_hex8_plastic.py::solve_plastic` on a **2x2x8 quarter-model mesh** (32 elements, 10 load steps, u_z=1.0 mm at z1 — amended from the original "e.g. 8x8x32, 4x4x16 at 20 steps" proposal)
- Store `displacement[]`, `force_history[]`, `disp_history[]`, `mesh_params`, `material_params`, and residual history in `tests/golden/necking_bar_reference.npz`
- **Mesh-density amendment (2026-04-10):** The original 4x4x16 + 20-step configuration required ~90 min of pure-numpy reference-solver time per golden regeneration, making it infeasible to refresh on every ref-solver change. The 2x2x8/10-step snapshot finishes in ~30 s while still exhibiting necking localization and plastic strain evolution (max alpha ≈ 0.55). This is a regression snapshot of the handwritten kernel, not a fine-mesh convergence study; literature-value comparison (Simo & Hughes 1998, 2%) is performed downstream at the benchmark level in P3-4.
- Avoids needing to digitize Simo & Hughes (1998) figures. The 2% tolerance validates generated solver matches reference solver on the same mesh density.
...
- [ ] Rigid body: zero internal force after 30-degree rotation + translation, norm < 1e-12
...
4. **SVK + finite rotation**: Already verified working -- SVK in Total Lagrangian gives E=0 for pure rotation, so f_int=0 exactly regardless of angle. Tests in `test_patch_test.py:374` pass with 30-degree rotation.
